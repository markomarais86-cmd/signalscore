-- Drop and recreate the function with batch processing support
DROP FUNCTION IF EXISTS public.bulk_match_all_leads(UUID);
...
DECLARE
  ...
  v_new_domain TEXT;
BEGIN
  -- Count unlinked leads (limited by batch size)
  ...
  -- Phase 1: Match leads to EXISTING accounts by domain (batch limited)
  ...
  -- Phase 2: Get distinct domains that need new accounts (from remaining unlinked)
  ...
  -- Phase 4: Link remaining unlinked leads to newly created accounts
  ...
  RETURN jsonb_build_object(
    ...
    'has_more', EXISTS (
      SELECT 1 FROM "Leads" 
      WHERE org_id = p_org_id AND account_id IS NULL 
      LIMIT 1
    )
  );
END;
