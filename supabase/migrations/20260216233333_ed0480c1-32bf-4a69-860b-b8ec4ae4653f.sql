...
-- Update search_list_builder to support custom attribute filtering
        ...
        SELECT bool_and(
          CASE 
            WHEN ca.key LIKE '%\_min' ESCAPE '\' THEN
              (rn.custom_attributes ->> REPLACE(ca.key, '_min', ''))::numeric >= ca.value::numeric
            WHEN ca.key LIKE '%\_max' ESCAPE '\' THEN
              (rn.custom_attributes ->> REPLACE(ca.key, '_max', ''))::numeric <= ca.value::numeric
            ELSE
              rn.custom_attributes ->> ca.key ILIKE '%' || ca.value || '%'
          END
        )
...
-- Add GIN index on custom_attributes for JSONB queries
CREATE INDEX IF NOT EXISTS idx_accounts_custom_attributes ON accounts USING GIN (custom_attributes);
