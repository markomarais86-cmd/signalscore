-- Phase 6.2: Fix Lead-to-Account Matching Function
-- This function has been updated to:
-- 1. Better extract domains from website AND email fields
-- 2. Handle large volumes more efficiently
-- 3. Normalize revenue ranges and employee counts
-- 4. Auto-score newly created accounts
...
BEGIN
  ...
  -- Step 2: Create new accounts for unmatched leads with valid domains
  ...
  RAISE NOTICE 'Created % new accounts', v_created_count;

  -- Step 3: Link leads to newly created accounts
  WITH lead_domains AS (
    SELECT 
      l.id,
      COALESCE(
  ...
  RAISE NOTICE 'Re-linked leads to accounts';
