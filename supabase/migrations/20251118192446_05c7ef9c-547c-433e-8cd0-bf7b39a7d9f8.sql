-- Fix ambiguous column reference error by qualifying CTE columns
...
BEGIN
  ...
  SELECT
    (SELECT COUNT(*)::bigint FROM filtered_accounts),
    (SELECT COUNT(*)::bigint FROM "Leads" l INNER JOIN filtered_accounts fa ON l.account_external_id = fa.external_id WHERE l.org_id = p_org_id),
    COALESCE(sm.high_fit_accounts, 0)::bigint,
    COALESCE(sm.medium_fit_accounts, 0)::bigint,
    COALESCE(sm.low_fit_accounts, 0)::bigint,
    COALESCE(cm.campaign_ready_accounts, 0)::bigint,
    COALESCE(cm.campaign_ready_contacts, 0)::bigint
