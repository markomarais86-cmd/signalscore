-- Fix geography distribution for "All Sources" using CTEs instead of temp tables
...
BEGIN
  ...
  -- ALL SOURCES: Use CTE instead of temp table to combine both sources
  IF p_source_filter = 'all' THEN
    ...
    combined AS (
      -- Union and sum both sources
      SELECT country, SUM(count) as total_count
      FROM (
        SELECT country, count FROM crm_geo
        UNION ALL
        SELECT country, count FROM external_geo
      ) all_geo
      GROUP BY country
    )
