CREATE OR REPLACE FUNCTION public.calculate_account_score(p_account_external_id text, p_org_id text, p_icp_id text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
...
BEGIN
    ...
    IF icp_rec.vertical_filters ? 'segments' AND jsonb_typeof(icp_rec.vertical_filters -> 'segments') = 'array' THEN
      ...
      BEGIN
        ...
        LOOP
          ...
          -- Match on bed_range — ALWAYS count as a criterion if defined
  ...
  v_breakdown := v_breakdown || jsonb_build_object('factor', 'vertical', 'score', v_vertical_score, 'match', v_vertical_score > 0, 'value', 'custom_attributes', 'missing_required_vertical', v_missing_required_vertical);
  ...
  -- Cap at Band C (max 69) if required vertical data (bed_count) is missing
  ...
  -- Also cap overall at 69 if missing required vertical
  IF v_missing_required_vertical THEN
    v_overall_score := LEAST(v_overall_score, 69);
  END IF;
