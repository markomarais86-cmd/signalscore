-- Fix match_leads_to_accounts_fast by removing ON CONFLICT and using WHERE NOT EXISTS
-- This avoids the functional index conflict issue entirely
...
BEGIN
  ...
  -- Step 2: Create new accounts - using WHERE NOT EXISTS to avoid duplicates
  ...
  -- Step 3: Link remaining unlinked leads
  WITH lead_domains AS (
    SELECT 
      l.id,
      COALESCE(
