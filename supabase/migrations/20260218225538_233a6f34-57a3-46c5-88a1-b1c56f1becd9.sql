-- Update revenue tiering in both SQL functions to use ILIKE for flexible matching
...
BEGIN
  ...
  IF account_rec.industry_norm IS NOT NULL AND icp_rec.industries IS NOT NULL THEN
    ...
    LOOP
      IF lower(account_rec.industry_norm) LIKE '%' || lower(v_industry) || '%' 
        ...
        v_industry_score := 20; v_matches := v_matches + 1; EXIT;
      END IF;
    END LOOP;
  ...
  IF account_rec.employee_count IS NOT NULL AND icp_rec.company_sizes IS NOT NULL AND array_length(icp_rec.company_sizes, 1) > 0 THEN
    DECLARE v_min_size integer; v_max_size integer; v_sizes integer[];
    BEGIN
  ...
  IF account_rec.country IS NOT NULL AND icp_rec.geographies IS NOT NULL THEN
    ...
    LOOP
      IF lower(account_rec.country) = lower(v_geo) THEN
        v_geo_score := 10; v_matches := v_matches + 1; EXIT;
      END IF;
    END LOOP;
  END IF;
  ...
  IF account_rec.revenue_range IS NOT NULL AND icp_rec.revenue_ranges IS NOT NULL THEN
    ...
    LOOP
      IF account_rec.revenue_range = v_rev THEN
        v_rev_lower := lower(account_rec.revenue_range);
        IF v_rev_lower LIKE '%$1b%' OR v_rev_lower LIKE '%$5b%' OR v_rev_lower LIKE '%$10b%'
           OR v_rev_lower LIKE '%$250m%' OR v_rev_lower LIKE '%$500m%' THEN
          v_revenue_score := 20;
        ELSE
          v_revenue_score := 12;
        END IF;
        v_matches := v_matches + 1; EXIT;
      END IF;
    END LOOP;
  END IF;
  ...
  IF icp_rec.vertical_filters IS NOT NULL AND icp_rec.vertical_filters ? 'segments' THEN
    ...
    IF jsonb_array_length(v_segments) > 0 THEN
      FOR v_seg IN SELECT * FROM jsonb_array_elements(v_segments) LOOP
        IF v_seg->>'bed_range' IS NOT NULL THEN v_any_seg_has_beds := true; EXIT; END IF;
      END LOOP;
      IF v_any_seg_has_beds AND v_bed_count IS NULL THEN v_missing_vertical := true; END IF;

      FOR v_seg IN SELECT * FROM jsonb_array_elements(v_segments) LOOP
        v_crit_total := 0; v_crit_matched := 0;
        ...
        IF v_bed_range IS NOT NULL THEN
          ...
          IF v_bed_count IS NOT NULL THEN
            ...
            IF v_bed_count >= v_min_beds AND v_bed_count <= v_max_beds THEN v_crit_matched := v_crit_matched + 2; END IF;
          END IF;
        END IF;
        ...
        IF v_size_label != '' AND v_ec IS NOT NULL THEN
          ...
          IF v_ec >= v_size_min AND v_ec <= v_size_max THEN v_crit_matched := v_crit_matched + 1; END IF;
        END IF;
        IF v_crit_total > 0 THEN
          DECLARE v_score integer;
          BEGIN
            v_score := round(30.0 * v_crit_matched / v_crit_total);
            IF v_score > v_best_seg_score THEN v_best_seg_score := v_score; v_matched_segment := v_seg->>'name'; END IF;
          END;
        END IF;
      END LOOP;
  ...
  RETURN jsonb_build_object('overall', v_total_score, 'fit', v_fit_score, 'intent', v_intent_score, 'reachability', 70,
    'missing_required_vertical', v_missing_vertical, 'matched_segment', v_matched_segment);
END;
$function$;

-- Readonly version (identical logic)
...
BEGIN
  ...
  IF account_rec.industry_norm IS NOT NULL AND icp_rec.excluded_industries IS NOT NULL THEN
    FOREACH v_ex_ind IN ARRAY icp_rec.excluded_industries LOOP
      IF lower(account_rec.industry_norm) LIKE '%' || lower(v_ex_ind) || '%' THEN v_excluded := true; EXIT; END IF;
    END LOOP;
    IF v_excluded THEN RETURN jsonb_build_object('overall', 0, 'fit', 0, 'intent', v_intent_score, 'reachability', 0); END IF;
  END IF;

  IF account_rec.industry_norm IS NOT NULL AND icp_rec.industries IS NOT NULL THEN
    FOREACH v_industry IN ARRAY icp_rec.industries LOOP
      IF lower(account_rec.industry_norm) LIKE '%' || lower(v_industry) || '%' 
         OR lower(v_industry) LIKE '%' || lower(account_rec.industry_norm) || '%' THEN
        v_industry_score := 20; v_matches := v_matches + 1; EXIT;
      END IF;
    END LOOP;
    IF v_industry_score = 0 AND icp_rec.company_keywords IS NOT NULL AND array_length(icp_rec.company_keywords, 1) > 0 THEN
      FOREACH v_keyword IN ARRAY icp_rec.company_keywords LOOP
        IF lower(account_rec.industry_norm) LIKE '%' || lower(v_keyword) || '%' THEN
          v_industry_score := 20; v_matches := v_matches + 1; EXIT;
        END IF;
      END LOOP;
  ...
  IF account_rec.country IS NOT NULL AND icp_rec.geographies IS NOT NULL THEN
    FOREACH v_geo IN ARRAY icp_rec.geographies LOOP
      IF lower(account_rec.country) = lower(v_geo) THEN v_geo_score := 10; v_matches := v_matches + 1; EXIT; END IF;
    END LOOP;
  END IF;

  IF account_rec.revenue_range IS NOT NULL AND icp_rec.revenue_ranges IS NOT NULL THEN
    FOREACH v_rev IN ARRAY icp_rec.revenue_ranges LOOP
      IF account_rec.revenue_range = v_rev THEN
        v_rev_lower := lower(account_rec.revenue_range);
        IF v_rev_lower LIKE '%$1b%' OR v_rev_lower LIKE '%$5b%' OR v_rev_lower LIKE '%$10b%'
           OR v_rev_lower LIKE '%$250m%' OR v_rev_lower LIKE '%$500m%' THEN
          v_revenue_score := 20;
        ELSE v_revenue_score := 12;
        END IF;
        v_matches := v_matches + 1; EXIT;
      END IF;
    END LOOP;
  ...
  v_fit_score := v_total_score;
  IF v_missing_vertical THEN
    v_total_score := LEAST(v_total_score, 69); v_fit_score := LEAST(v_fit_score, 69);
  END IF;
