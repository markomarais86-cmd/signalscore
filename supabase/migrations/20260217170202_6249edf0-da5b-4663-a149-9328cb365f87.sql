CREATE OR REPLACE FUNCTION public.get_filtered_accounts(p_org_id uuid, p_cursor timestamp with time zone DEFAULT NULL::timestamp with time zone, p_limit integer DEFAULT 25, p_search_term text DEFAULT NULL::text, p_industry text DEFAULT NULL::text, p_country text DEFAULT NULL::text, p_data_source text DEFAULT NULL::text, p_fit_min integer DEFAULT NULL::integer, p_fit_max integer DEFAULT NULL::integer, p_campaign_ready boolean DEFAULT false, p_sort_field text DEFAULT 'updated_at'::text, p_sort_direction text DEFAULT 'desc'::text)
 RETURNS TABLE(external_id text, name text, industry_norm text, country text, city text, state_province text, employee_count integer, revenue_range text, domain text, linkedin_url text, data_source text, updated_at timestamp with time zone, enriched_at timestamp with time zone, enrichment_overall_score numeric, icp_qualified boolean, deep_research_requested boolean, deep_research_completed_at timestamp with time zone, tech_stack text[], total_raised_usd numeric, last_funding_round text, last_funding_date date, total_count bigint)
 LANGUAGE plpgsql
 SECURITY DEFINER
