...
-- Fix column name from overall_fit_score to overall
...
BEGIN
  ...
  -- Fit distribution (high/medium/low scores) - FIXED: use 'overall' not 'overall_fit_score'
  IF p_icp_id IS NOT NULL THEN
    SELECT 
      COUNT(*) FILTER (WHERE s.overall >= 70) as high_fit,
      COUNT(*) FILTER (WHERE s.overall >= 40 AND s.overall < 70) as med_fit,
      COUNT(*) FILTER (WHERE s.overall < 40) as low_fit,
      COUNT(*) FILTER (WHERE s.overall >= 70 AND a.data_source = 'crm') as high_crm,
      COUNT(*) FILTER (WHERE s.overall >= 70 AND a.data_source IN ('database', 'api', 'enrichment')) as high_db
  ...
  ELSIF p_source_filter = 'crm' THEN
    SELECT 
      ...
      COUNT(*) FILTER (WHERE s.overall >= 40 AND s.overall < 70) as med_fit,
      COUNT(*) FILTER (WHERE s.overall < 40) as low_fit,
      COUNT(*) FILTER (WHERE s.overall >= 70) as high_crm,
      0 as high_db
    INTO v_high_fit, v_medium_fit, v_low_fit, v_high_fit_crm, v_high_fit_db
    FROM scores s
    JOIN accounts a ON s.account_external_id = a.external_id AND s.org_id = a.org_id
  ...
  ELSIF p_source_filter = 'database' THEN
    SELECT 
      ...
      COUNT(*) FILTER (WHERE s.overall < 40) as low_fit,
      0 as high_crm,
      COUNT(*) FILTER (WHERE s.overall >= 70) as high_db
    INTO v_high_fit, v_medium_fit, v_low_fit, v_high_fit_crm, v_high_fit_db
    FROM scores s
    JOIN accounts a ON s.account_external_id = a.external_id AND s.org_id = a.org_id
