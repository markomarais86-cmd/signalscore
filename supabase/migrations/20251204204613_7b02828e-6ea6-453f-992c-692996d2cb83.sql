-- Fix to properly handle domain uniqueness constraint
...
BEGIN
  ...
  -- Phase 1: Match leads to EXISTING accounts by domain (using normalize_domain_text if it exists, otherwise LOWER)
  ...
  -- Phase 2: Get distinct domains that need new accounts (excluding any existing by normalized domain)
  ...
  WHERE domain IS NOT NULL 
    ...
    AND NOT EXISTS (
      ...
      WHERE a.org_id = p_org_id 
        AND LOWER(TRIM(a.domain)) = domains.domain
    );

  -- Phase 3: Create accounts for new domains - skip conflicts silently
  IF v_domains_to_create IS NOT NULL AND array_length(v_domains_to_create, 1) > 0 THEN
    BEGIN
      ...
      ON CONFLICT DO NOTHING;  -- Skip ALL conflicts (any unique constraint)
      
      GET DIAGNOSTICS v_accounts_created = ROW_COUNT;
    EXCEPTION WHEN unique_violation THEN
      -- Just continue if there are duplicates
