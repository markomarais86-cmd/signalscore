-- Fix match_leads_to_accounts_fast to handle duplicate domains properly
-- This fixes the constraint violation error when matching leads to accounts
...
BEGIN
  ...
  -- Step 2: Create new accounts for unmatched leads (only for domains that don't exist yet)
  WITH new_accounts_data AS (
    ...
    ORDER BY normalized_domain, l.id DESC  -- Take most recent lead data per domain
  ),
  filtered_new_accounts AS (
    -- Only include domains that truly don't exist yet
    SELECT nad.*
  ...
  inserted_accounts AS (
    ...
    FROM filtered_new_accounts fna
  ...
  -- Step 3: Link ALL remaining unlinked leads to their accounts (existing or newly created)
  WITH lead_domains AS (
    SELECT 
      l.id,
      COALESCE(
