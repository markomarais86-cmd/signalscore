-- Fix match_leads_to_accounts_fast to handle domain constraint conflicts and add Step 3 logging
...
BEGIN
  ...
  RAISE NOTICE 'Step 1: Matched % leads to existing accounts', v_matched_count;
  ...
  -- Pre-filter to avoid inserting domains that already exist
  ...
  inserted_accounts AS (
    ...
    -- Handle domain constraint conflicts gracefully
    ON CONFLICT ON CONSTRAINT idx_accounts_unique_domain_per_org DO NOTHING
  ...
  RAISE NOTICE 'Step 2: Created % new accounts (Source: %)', v_created_count, CASE WHEN p_is_external_db THEN 'database' ELSE 'crm' END;

  -- Step 3: Link remaining unlinked leads (safety net)
  WITH lead_domains AS (
    SELECT 
      l.id,
      COALESCE(
  ...
  RAISE NOTICE 'Step 3: Linked % remaining leads to existing accounts', v_step3_linked;
