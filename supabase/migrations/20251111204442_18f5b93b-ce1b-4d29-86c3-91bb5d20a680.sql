-- Fix: For "all" sources, count ALL accounts from accounts table as CRM (don't filter by data_source)
...
BEGIN
  ...
  -- ALL SOURCES: Combine ALL accounts from accounts table + external database
  IF p_source_filter = 'all' THEN
    ...
    RETURN QUERY
    WITH crm_geo AS (
      -- Count ALL accounts in accounts table (these are CRM accounts)
      SELECT 
      ...
      WHERE a.org_id = p_org_id
        AND a.country IS NOT NULL
        AND a.country != ''
        -- NO data_source filter for "all" - count everything
      GROUP BY a.country
    ),
    external_geo AS (
      SELECT 
